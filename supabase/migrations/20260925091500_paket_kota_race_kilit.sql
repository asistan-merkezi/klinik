-- Kod incelemesi bulgusu: paket kişi kotası kontrolü (kisi_kotasi) hiçbir
-- kilit almadan "kaç farklı hasta bu paketi aldı" sayıp sınırla
-- karşılaştırıyordu. İki eşzamanlı odeme_olustur çağrısı (aynı paketi farklı
-- İKİ YENİ hastaya satan) kota tam dolmadan hemen önce aynı anda gelirse,
-- ikisi de sayımı "kota dolmadı" olarak görüp geçebiliyor — kota bir kişi
-- aşılabiliyordu (TOCTOU). randevu_gelis_isaretle zaten paket_satis satırını
-- düşerken FOR UPDATE ile kilitliyordu, aynı disiplin burada da uygulanıyor:
-- kota kontrolünden önce paket satırının kendisi FOR UPDATE ile kilitlenip
-- aynı pakete satış yapan eşzamanlı transaction'lar serileştiriliyor.
--
-- Fonksiyonun geri kalanı BİREBİR AYNI (20260815090000_paket_satisi_borc_olarak.sql) —
-- tek fark: paket SELECT'ine FOR UPDATE eklendi.
...
BEGIN
  ...
  LOOP
    ...
    ELSIF v_kalem->>'tur' = 'paket' THEN
      -- FOR UPDATE: aynı pakete eşzamanlı satış yapan başka bir transaction
      -- varsa burada bekletiliyor — kota sayımı ve satır ekleme artık aynı
      -- paket için serileşiyor (kota_doldu TOCTOU'sunun düzeltmesi).
...
-- Kontrol:
-- SELECT prosrc FROM pg_proc WHERE proname = 'odeme_olustur'; -- 'FOR UPDATE' paket SELECT'inde görünmeli
