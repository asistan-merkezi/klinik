-- Paket tanımına "kişi kotası" eklendi (kullanıcı isteği, 2026-08-14): bu
-- paketi aynı anda kullanan farklı (distinct) hasta sayısının üst sınırı.
-- Boş bırakılırsa (NULL) sınırsız. Kotaya ulaşıldığında YENİ bir kişiye satış
-- reddedilir — daha önce bu paketten satın almış biri (hangi durumda olursa
-- olsun, "yenileme") kotadan bağımsız her zaman satın alabilir, çünkü kural
-- "yeni bir kişiye satış" içindi, mevcut/dönen katılımcıya değil.
ALTER TABLE paket ADD COLUMN kisi_kotasi integer CHECK (kisi_kotasi IS NULL OR kisi_kotasi > 0);

-- ==================== odeme_olustur: paket satışında kişi kotası kontrolü ====================
...
-- Kontrol:
-- SELECT column_name FROM information_schema.columns WHERE table_name = 'paket' AND column_name = 'kisi_kotasi';
