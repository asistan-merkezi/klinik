-- Kullanıcı kararı: borç kapatma akışında ayrı bir "kredi" hareketi
-- YARATILMASIN — sadece borç ve ödeme olsun. İskonto girildiğinde ORİJİNAL
-- borç satırının kendisi güncellensin (tur='odeme', tutar=net tutar),
-- ikinci bir satır eklenmesin. Bu, önceki turun "borcu ayrı bir kredi
-- satırıyla kapat" kararının BİLİNÇLİ OLARAK yerini alıyor (append-only
-- ledger felsefesinden bu tek etkileşimli akış için bilerek sapılıyor —
-- kullanıcı doğrudan bunu istedi).
--
-- Bakiye etkisi hâlâ doğru: v_hasta_ozet/hareketleriGorunumeCevir'in
-- "kredi:+, borc:-, else:0" kuralına göre tur 'borc'tan 'odeme'ye
-- geçtiğinde satırın bakiyeye etkisi otomatik olarak -tutar'dan 0'a
-- düşüyor (borç kapandı, ekstra bir hareket gerekmeden).
...
BEGIN
  ...
  -- Ayrı bir 'kredi' satırı YOK — orijinal borç satırının kendisi ödemeye
  -- dönüştürülüyor (tur='odeme', tutar=net tutar). Bakiyeye etkisi bu
  -- sayede otomatik olarak -tutar'dan 0'a düşüyor (borç kapandı).
