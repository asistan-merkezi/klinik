...
--
-- Not: İlk sürüm CREATE TEMP TABLE kullanıyordu — Supabase SQL Editor'de
-- "relation does not exist" hatası verdi (session'a bağlı geçici tablo,
-- editör isteği farklı bağlantı/oturumlara bölüyor olmalı). Bu sürüm hiç
-- geçici tablo kullanmıyor; personelin eski pozisyon adını taşımak için
-- personel.gorev'i geçici bir "__yeni__<hedef ad>" işaretleyicisiyle
-- kullanıyor (tek bir DO bloğu = tek ifade, PL/pgSQL adım sırası garanti).
do $$
declare
  v_klinik_id uuid := '7d259889-5bd8-4702-9e39-d86488234667';
begin
  -- 1) Personelin ŞU ANKİ pozisyonunun adını (gerçek pozisyon_id üzerinden,
  --    gorev serbest metnine güvenmeden) yeni katalogdaki karşılığına
  --    eşleyip geçici olarak gorev'e yaz — eski pozisyon satırları
  --    silinmeden önce bu bilgiyi bir yerde tutmamız lazım.
  update personel p
  set gorev = '__yeni__' || case poz.ad
  ...
  where poz.id = p.pozisyon_id
    and p.klinik_id = v_klinik_id
    and poz.ad in ('Spor Fizyoterapisti', 'Manuel Terapist', 'terapist', 'Fizyoterapist', 'prfo');

  -- 2) Eski klinik pozisyonlarını ve platform şablonlarını temizle.
  delete from pozisyonlar where klinik_id = v_klinik_id;
  ...
  -- 4) Mevcut kliniğe aynı katalogdan pozisyon satırları — sadece
  --    hâlihazırda personel bağlı olan 5 tanesi aktif.
  ...
  -- 5) Personeli yeni pozisyonlara bağla + gorev'i geçici işaretleyiciden
  --    temizleyip gerçek pozisyon adıyla eşitle.
  update personel p
  set pozisyon_id = poz.id,
      gorev = poz.ad
  from pozisyonlar poz
  where p.klinik_id = v_klinik_id
    and poz.klinik_id = v_klinik_id
    and p.gorev = '__yeni__' || poz.ad;
end $$;
