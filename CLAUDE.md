...
**Roller**: `super_admin`, `klinik_admin`, `resepsiyon`, `terapist`, `muhasebe` (dar kapsam: fatura kesme + bordro salt-okunur; hasta/randevu/personel yönetimi ve hassas veri bilinçli kapalı). Sidebar/hub seviyesinde artık merkezi bir modül-izin fonksiyonu var (`lib/auth/roles.ts` → `canAccessModule`, bkz. Yetkilendirme) ama alt sayfaların çoğu hâlâ kendi `rol IN (...)` kontrolünü tekrarlıyor — merkeze taşınmamış (bkz. Teknik Borç).

- **Klinik**: `klinik` · `kullanici` (2026-09-20: `allowed_modules` jsonb + `custom_permissions_enabled` bool + `permissions_version` int eklendi — çekirdek şemadan beri bu tabloya kolon ekleyen ilk migration; bir `BEFORE UPDATE` trigger'ı `rol`/bu iki alanı yalnız `klinik_admin`/`super_admin` aktörle değiştirtir, `id=auth.uid()` satır-seviyesi RLS'inin kolon-kör olmasına karşı savunma) · `klinik_ayarlar` (tüm konfig tek jsonb `ayarlar`: gizlilik modu, no-show, features, `qr_kodlari`, tablet ayarları — read-modify-write ile güncellenir; **`sidebar_gizli` anahtarı 2026-09-20'de terk edildi**, bkz. Yetkilendirme).
...
### Teknik Borç: Sidebar/route erişimi henüz tam merkezi değil (Faz 2 bekliyor)
2026-09-20'de Yetkilendirme mimarisi yeniden kuruldu: `lib/auth/roles.ts` (istemci-güvenli `MODULE_TREE`/`ROUTE_TO_MODULE`/`canAccessModule`/`resolveHubRedirect`) + `lib/auth/roles-server.ts` (`kullaniciModulleriGetir`/`erisimKontrolEt`, server-only) TEK kaynak — Sidebar (`components/panel/sidebar.tsx`) ve 4 hub sayfası (`/panel/finans`, `/panel/yonetim`, `/panel/ayarlar`, `/panel/finans/gelirler-takibi`) artık buradan besleniyor, eski departman bazlı `klinik_ayarlar.ayarlar.sidebar_gizli` mekanizması TAMAMEN kaldırıldı. **Ama kapsam kısmi**: geri kalan ~30 sayfadaki dağınık `kullanici?.rol === "..."` kontrolleri (ör. personel/[id]'de `muhasebeErisimi`) henüz `canAccessModule`'e taşınmadı — link Sidebar'da görünür/gizli olabilir ama sayfanın kendi eski rol kontrolü hâlâ ayrı çalışıyor (aynı sınıf eski gap, sadece anahtar uzayı roldan modül anahtarına kaydı). Kalıcı çözüm: her sayfayı kademeli olarak `erisimKontrolEt(moduleKey)` çağırmaya geçirmek.
