...
- **Cron** (`vercel.json`): `personel-donem-otomatik-kapat` ayın 1'i 03:00 UTC (bir önceki ayın maaş hakedişini TÜM aktif personel için otomatik kapatır, bkz. personel modülü CLAUDE.md). `mesaj-kuyruk-isle` (5 dk) ve `kredi-senkron` (saatlik) route'ları kodda var ama **2026-09-16'da fark edildi: `vercel.json`'da hiçbir zaman `crons` anahtarı yoktu** — muhtemelen hiç production'da çalışmadılar, teyit edilip eklenmeleri gerekiyor. `CRON_SECRET` Bearer ile doğrulanır — **aynı değer Vercel env'ine de girilmeli**, yoksa prod'da hep 401. Hobby planında 5 dk sıklık reddedilebilir (plan teyit edilmedi).
...
- Cron'ların Vercel plan limitine takılıp takılmadığı ve `CRON_SECRET`'in Vercel env'ine girilip girilmediği teyit edilmeli. `20260916100000_personel_donem_kapat_otomasyon.sql` migration'ı henüz Supabase'e uygulanmadı — kullanıcı tarafından uygulanacak.
