...
Sprint 2 (aktif): ödeme/paket şeması migration'a yazıldı ve uygulandı (`islem_kategori`, `islem_tanimi`, `paket`, `odeme`, `paket_satis`, `odeme_kalemi`, `odeme_satiri`, `musteri_bakiye_hareket` + RLS — fiyat kataloğunu klinik_admin yönetir, ödeme/paket_satis/bakiye hareketini klinik_admin+resepsiyon işler). Panelde tamam olan ekranlar: İşlem Tanımları (`/panel/islemler`) ve Paketler (`/panel/paketler`) — ikisi de liste/oluşturma/düzenleme/aktif-pasif. Sıradaki: paket satış akışı (müşteriye paket satma, `paket_satis` + `musteri_bakiye_hareket`) ve Ödeme Alma ekranı (fiyat seçimi → iskonto → faturalı/faturasız → ödeme al) — tablolar hazır ama bu iki ekran henüz yok. Ardından Paraşüt + WhatsApp entegrasyonu.
...
Son güncelleme: 2026-07-27 (sprint 2: ödeme/paket şeması + İşlem Tanımı ve Paketler ekranları tamamlandı)
