...
## Yol Haritası / Durum
**Çalışır durumda**: randevu/çizelge + check-in + oda tableti (realtime) · hasta CRUD + Hasta Detay 4 route'lu hub (`/kisisel`, `/randevu`, `/tedavi` — içinde 2 iç-sekme: Tedavi & Anamnez + Gelişim & Ölçümler; `/cari` terapiste kapalı) · Hasta Detayı başlığı Talep ve Öneriler (randevu talebi/iptali + terapist/randevu yorumu, panelden — bkz. Hasta Portalı) · belge/foto yükleme (tek POST `/api/hasta-belge/foto-yukle`, Canvas+WebP 1280px/%85) · cari borç-ödeme · paketler (kota + katılımcı + yenile) · tedavi tanımları + kademeli fiyat/iskonto · personel modülü · hasta portalı · 4 QR akışı + seans anketi · arşiv içe aktarma · mesajlaşma Faz 1-2 + merkez istemcisi · 2 PDF formu · Destek/Talep-Şikayet · tasarım sistemi geçişi · Kasa/Banka (nakit+havale mutabakatı, bkz. Veri Modeli).
