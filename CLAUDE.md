...
- **Kasa/Banka** (`/panel/finans/kasa`, `/panel/finans/banka`, 2026-09-16): Kasa'nın kendi tablosu yok — bakiye `klinik_ayarlar.ayarlar.kasa.baslangic_tutari` + `hasta_bakiye_hareket`(nakit)/`klinik_harcama`(nakit)/`personel_hesap_hareket`(nakit, `odeme`/`avans`)/`nakit_banka_hareketi`(kasa bacağı) toplamından hesaplanır; Banka aynısını `banka_havalesi`/`havale` + `banka_hesap_id` ile hesap bazında yapar, `openingBalance` her zaman 0. Ortak render `components/panel/ledger-view.tsx` (dönem başı bakiye + running balance + tıkla-genişlet). "Kamusal Ödeme" bilinçli olarak sadece Banka'da (hep havale sayılır, `kamusal_odeme`'ye ödeme yöntemi kolonu eklenmedi) — Kasa'da yok. Hasta nakit iadesi (`tur='iade'`) ödeme yöntemi izlemiyor, kasa mutabakatına yansımaz (bilinçli, küçük boşluk).
