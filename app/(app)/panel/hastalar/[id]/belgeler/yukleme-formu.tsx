...
export function YuklemeDialogu({
  ...
  const { data: onam, isLoading: onamYukleniyor } = useKlinikFotoOnam(hastaId, kategori === "klinik_foto");
  const { data: mevcutGruplar } = useKarsilastirmaGruplari(hastaId, kategori === "klinik_foto");
  ...
  async function yuklemeyiBaslat() {
    ...
    if (herhangiBasarili) {
      queryClient.invalidateQueries({ queryKey: ["hasta_belge", hastaId, kategori] });
      queryClient.invalidateQueries({ queryKey: ["karsilastirma_gruplari", hastaId] });
    }
  }
...
function OnamAlButonu({ hastaId }: { hastaId: string }) {
  const queryClient = useQueryClient();
    ...
    <div className="flex flex-col gap-1.5">
      <Button
        ...
        onClick={async () => {
          setPending(true);
          setHata(null);
          const sonuc = await klinikFotoOnamKaydet(hastaId);
          setPending(false);
          if ("error" in sonuc) {
            setHata(sonuc.error);
            return;
          }
          queryClient.invalidateQueries({ queryKey: ["hasta_onam_klinik_foto", hastaId] });
        }}
        className="w-fit"
      >
