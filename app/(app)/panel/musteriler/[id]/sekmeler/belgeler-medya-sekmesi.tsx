...
import { BELGE_KATEGORI_ETIKETLERI, type BelgeKategori } from "@/types/musteri-belge";
import { VUCUT_BOLGE_ETIKETLERI } from "@/types/musteri-detay";
import { useMusteriBelgeler } from "../belgeler/queries";
...
export function BelgelerMedyaSekmesi({ musteriId, rol, aktif }: { musteriId: string; rol: string | null; aktif: boolean }) {
  ...
  const { data: belgeler, isLoading } = useMusteriBelgeler(musteriId, kategori, aktif);
  ...
  return (
    <div className="flex flex-col gap-4">
      <Card>
        <CardContent className="flex flex-col gap-4">
          ...
          {isLoading ? (
            ...
            <RadyolojiGrid belgeler={filtrelenmis} musteriId={musteriId} rol={rol} onYukle={() => setYuklemeAcik(true)} />
          ) : kategori === "klinik_foto" ? (
      ...
      {yuklemeAcik && (
        <YuklemeDialogu musteriId={musteriId} varsayilanKategori={kategori} onKapat={() => setYuklemeAcik(false)} />
      )}
    </div>
  );
}
