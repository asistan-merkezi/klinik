...
function PeriyodikSatir({ musteriId, periyodik }: { musteriId: string; periyodik: PeriyodikRandevuSatir }) {
  const [duzenleniyor, setDuzenleniyor] = useState(false);
  const [uzatVarsayilan, setUzatVarsayilan] = useState(false);
  const guncelleAction = periyodikRandevuGuncelle.bind(null, periyodik.id, musteriId);
          ...
          <Button
            ...
            onClick={() =>
              startIptalTransition(() => {
                void periyodikRandevuIptalEt(periyodik.id, musteriId);
              })
            }
          >
            İptal et
            ...
            <Button
              ...
              onClick={() =>
                startYenilenmeyecekTransition(() => {
                  void periyodikRandevuYenilenmeyecek(periyodik.id, musteriId);
                })
              }
            >
  ...
  return (
    <ul className="flex flex-col divide-y divide-border">
      {periyodikRandevular.map((p) => (
        <PeriyodikSatir key={p.id} musteriId={musteriId} periyodik={p} />
      ))}
    </ul>
  );
}
