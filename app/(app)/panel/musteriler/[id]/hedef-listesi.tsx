...
import { useMusteriHedefler } from "./queries";
import { musteriHedefEkle } from "./actions";

const HEDEF_TIPI_SECENEKLERI = Object.entries(HEDEF_TIPI_ETIKETLERI).map(([value, label]) => ({ value, label }));

function ilerlemeOrani(hedef: MusteriHedef, sonVasSkoru: number | null): number | null {
...
export function HedefListesi({
  ...
  const { data: hedefler, isLoading } = useMusteriHedefler(musteriId, aktif);
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {formAcik && <HedefFormu musteriId={musteriId} onKapat={() => setFormAcik(false)} />}
...
function HedefFormu({ musteriId, onKapat }: { musteriId: string; onKapat: () => void }) {
  const ekleAction = musteriHedefEkle.bind(null, musteriId);
