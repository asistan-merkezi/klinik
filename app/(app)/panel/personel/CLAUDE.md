...
**`hakedis` elle eklenemez** — hem `personel_hesap_hareket_ekle` RPC'si (manuel ekleme, klinik_admin-only) bunu reddeder hem de tablo CHECK'i (`kaynak_id` dolu olmayan bir `hakedis` satırı hiç var olamaz). Yazma yolu dönem kapanışı: `donemKapat` server action'ı (`puantaj-cetveli/actions.ts`) `personel_puantaj_donem_kapat` RPC'sinden dönen `donem_id`'yi kullanıp `lib/personel/hakedis.ts`'teki `hakedisHesapla()`'yı ÇAĞIRIR (formül TEK yerde, SQL'de tekrarlanmıyor) ve sonucu `personel_hesap_hareket_donem_ekle(donem_id, taban, prim)` RPC'sine geçirir — bu RPC idempotent'tir (`zaten_islendi` — aynı döneme ikinci kez hakediş yazılamaz). "Fazla mesai" (`mesai` türü) hâlâ `personel_puantaj_donem_kapat`'ın kendi içinde (SQL'de) yazılıyor, taşınmadı — sadece hedef tablosu değişti.

**Otomatik dönem kapanışı** (2026-09-16): `donemKapat`'ın klinik_admin'in elle tıklamasıyla çağırdığı AYNI iki RPC, her ayın 1'inde `/api/cron/personel-donem-otomatik-kapat` cron'undan (`vercel.json`, `0 3 1 * *`) service_role ile TÜM kliniklerdeki TÜM aktif (ve `puantaj_modu != 'takipsiz'`) personel için otomatik çağrılır — bir önceki ayın maaş hakedişi elle tıklamaya gerek kalmadan bakiyeye düşer. Formül/idempotency AYNI kaldı, sadece `personel_puantaj_donem_kapat` + `personel_hesap_hareket_donem_ekle` RPC'lerinin yetki kontrolüne `auth.role() = 'service_role'` yolu eklendi (`mesaj_kredi_senkronla`'daki aynı desen). Dönem zaten kapalıysa (manuel kapatılmış veya cron ikinci kez çalışmış) sessizce atlanır — "hakediş elle eklenemez" kuralı bozulmuyor, tetikleyici klinik_admin tıklaması yerine zamanlanmış görev.
...
Tek route (`/panel/personel/puantaj-cetveli`), tüm klinik için aylık matris — satırlar `pozisyon.puantaj_modu != 'takipsiz'` olan aktif personel (varsayılan 9 şablonun hiçbiri `takipsiz` değil, bu yüzden bugün itibarıyla tüm aktif personel görünüyor — `takipsiz` sadece Özel Pozisyon formuyla seçilebilen, bir personeli cetvelden tamamen çıkarma seçeneği) VE `personel.ise_giris_tarihi` görüntülenen ayın bitişinden önce olan (ya da hiç girilmemiş — o zaman kısıtlama yok) personel; sütunlar ayın günleri. Bu ikinci filtre 2026-09-16'da eklendi — daha önce işe giriş tarihinden ÖNCEKİ aylarda da personel satırı görünüyordu (`ise_giris_tarihi` hiçbir yerde kontrol edilmiyordu). Hücre kodu (`lib/personel/puantaj-cetveli.ts` → `gunKoduHesapla`) öncelik sırası: gerçek `personel_puantaj` kaydı > `resmi_tatil` tablosu > kliniğin kendi hafta sonu ayarı (`klinik.cumartesi_baslangic`/`pazar_baslangic` NULL ise o gün "X") > boş. `kaynak='izin_talebi'` olan hücreler dialogda salt-okunur gösteriliyor (izin akışının otomatik yazdığı günlere elle dokunulamaz, düzeltme gerekiyorsa ilgili izin talebi üzerinden yapılmalı).
