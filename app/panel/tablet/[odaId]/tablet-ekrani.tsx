  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-6 p-8 text-center">
      {mevcut ? (
        ...
        <>
          <span className="text-sm uppercase tracking-widest text-zinc-400">Sıradaki</span>
        ...
        <span className="text-3xl font-semibold text-zinc-500">Oda boş</span>
      )}
          ...
          <ul className="flex flex-col divide-y divide-zinc-800 text-sm">
            {randevular.map((r) => (
              <li key={r.id} className="flex items-center justify-between py-2">
                <span className={r.durum === "iptal" ? "text-zinc-600 line-through" : ""}>
                ...
                <span className="text-zinc-400">{DURUM_ETIKET[r.durum]}</span>
              </li>
            ))}
          </ul>
        </div>
